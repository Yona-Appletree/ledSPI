...
static sock_t ns_open_listening_socket(union socket_address *sa) {
  ...
  int socketopt_err=0, bind_err=0, listen_err=0;

  if ((sock = socket(sa->sa.sa_family, SOCK_STREAM, 6)) != INVALID_SOCKET &&
      !(socketopt_err = setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, (void *) &on, sizeof(on))) &&
      !(bind_err = bind(
        sock,
        &sa->sa,
        sa->sa.sa_family == AF_INET ? sizeof(sa->sin) : sizeof(sa->sin6)
      )) &&
      !(listen_err = listen(sock, SOMAXCONN))) {
  ...
  if (sock == INVALID_SOCKET) {
    DBG(("socketopt_err=%d bind_err=%d listen_err=%d", socketopt_err, bind_err, listen_err));
  }
